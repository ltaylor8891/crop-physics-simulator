...
- `develop` feature program (`docs/DEVELOP_PROGRAM.md`). **Phase A complete** (conveyor legs toggle + diverter attachment with along-belt, across-belt, length, and angle controls; roadmap Stage 16, ADR-018, `fileVersion` 5). Phases B–G planned, not started. `main` is untouched.
...
- Save format: versioned JSON, schema-validated; current `fileVersion` 5 (on `develop`; `main` is at 3).
