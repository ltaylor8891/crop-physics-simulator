...
- Crops never spawned in production builds (status bar stuck on **PHYSICS LOADING…**): the crop pools were reset after binding because React runs child effects before the parent effect that configured them; dev builds were unaffected only because StrictMode's second effect pass re-bound the pools. Pool configuration now happens before binding and only resets on a real capacity change (KI-008).
- Production deploy hardening (superseded diagnosis of the above): build copies `rapier_wasm3d_bg.wasm` beside the JS bundle. Kept, though `rapier3d-compat` embeds the WASM and the copy is believed unnecessary.
...
- Conveyor diverter (develop, Phase A): an optional angled high-side wall that locks to the belt surface to deflect crop across it — position along the belt, length, and angle are configurable; length 0 = none. Rendered mesh plus a matching fixed collider (ADR-018).
